...
func Parse(notation string) (c int64, t time.Time, d time.Duration, err error) {
	splitTime := strings.Split(notation, "/")
	if len(splitTime) != 3 {
		err = fmt.Errorf("Notation not formatted correctly. Should look like: R/2014-03-08T20:00:00Z/PT2H")
		return
	}
	...
	if splitTime[0] == "R" {
		...
		c, err = strconv.ParseInt(strings.Split(splitTime[0], "R")[1], 10, 0)
		if err != nil {
			err = fmt.Errorf("Error converting repeat field to an int: %s", err)
			return
		}
	}

	// Schedule time
	t, err = time.Parse(time.RFC3339, splitTime[1])
	if err != nil {
		t, err = time.Parse(RFC3339WithoutTimezone, splitTime[1])
		if err != nil {
			err = fmt.Errorf("Error converting schedule time field to a time.Time: %s", err)
			return
		}
	}
	...
	// Interval time
	var dur *Duration
	dur, err = FromString(splitTime[2])
	if err != nil {
		err = fmt.Errorf("Error converting interval field to a iso8601.Duration: %s", err)
		return
	}
	d = dur.ToDuration()
	return
